...
def receiveMessage():
    while True:
        try:
            ...
            if message:
                print(message)  # Imprimir mensaje recibido
            else:
...
def sendMessage():
    while True:
        message = input()
        try:
            if message.lower() == '/salir':  # Comando para salir
                client.close()
                print("Conexión cerrada.")
                break
            client.send(f"{nickname}: {message}".encode('utf-8'))  # Enviar mensaje con apodo
        except Exception as e:
...
try:
    client.connect(('127.0.0.1', 3308))  # Asegúrate de que el puerto sea correcto
